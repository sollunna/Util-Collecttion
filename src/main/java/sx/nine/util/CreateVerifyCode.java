...
public class CreateVerifyCode {
    ...
    public CreateVerifyCode(int width, int height, int codeCount, int lineCount, String code) throws Exception {
        this.width = width;
        this.height = height;
        this.codeCount = codeCount;
        this.lineCount = lineCount;
     ...
     * 生成指定字符图片
     */
    private void creatImage(String code) throws Exception {
        if(StringUtils.isEmpty(code)){
            throw new Exception("验证码为空或已过期，请重新获取");
        }
        // 字体的宽度
        int fontWidth = width / codeCount;
        // 字体的高度
